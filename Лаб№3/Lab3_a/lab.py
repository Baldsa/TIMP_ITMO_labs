...
def def_up(password):  # Поднимаем защиту
    ...
    if "template.tbl.enc" in os.listdir():
        pyAesCrypt.decryptFile(
            "template.tbl.enc", "template.tbl", password, bufferSize
        )  # Если в прошлый раз файл template.tbl был зашифрован, то нужно его сначала расшифровать.
...
def def_down(password):
    ...
    for i in fi["files"]:
        for j in glob.glob(i + ".enc"):
            pyAesCrypt.decryptFile(
                j, ".".join(j.split(".")[0:-1]), password, bufferSize
            )
            os.remove(j)
...
def main():
    ...
    if not int(flag_start):
        key_str = input("Введите где хранится ваша цифровая подпись ")
    ...
    if "template.tbl.enc" in os.listdir():
        try:
            pyAesCrypt.decryptFile(
                "template.tbl.enc", "template.tbl", password, 64 * 1028
            )
        except:
            print("\nПароль неверен!!!\n")
            sys.exit()
